...
def run(file_in, file_out=None, chunks=10):
    ...
    blockchain = Blockchain(special_magic_key, db,
                            file_path=file_in, chunks=chunks)
    splitted_data = blockchain._chunk_data()
    blockchain.set_db(db)

    for chunk_index, bin_data in enumerate(splitted_data):
        data = blockchain.encrypt_block(bin_data)
        previous_block = blockchain.get_previous_block()
        previous_txid = previous_block['transaction_id']
        new_block = blockchain.create_block(data, previous_txid, chunk_index, title=blockchain.file)
